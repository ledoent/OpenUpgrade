...
def migrate(env, version):
    """Migrate 18.0 underscored VATEX codes to the 19.0 hyphenated form on
    account.tax.ubl_cii_tax_exemption_reason_code. Explicit value-by-value
    mapping (not a regex REPLACE) so unrelated codes can never be touched.

    The column is provided by account_edi_ubl_cii_tax_extension (|del| in 19.0),
    which may not be installed in a given DB — guard so we no-op when absent
    rather than crash with UndefinedColumn.
    """
